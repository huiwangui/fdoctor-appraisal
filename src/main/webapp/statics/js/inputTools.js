/**
 * 全选、不全选实现方法
 * @param tdId   checkbox所在td的id
 * @param allCheckId   全选那个checkbox的id
 */
function getAllCheck(tdId,allCheckId){
		    ...
		    function allchk() {  
		        ...
		        var chk = 0;  
		        $("#"+tdId+" :checkbox").each(function () {  
		            if ($(this).attr('id') != $("#"+allCheckId).attr('id') && this.checked) {  
		                chk++;  
		            }  
		        });  
		        if (chknum == chk) {//全选  
...
/**
 * 设置checkbox回显
 * @param data  传入的数据值，必须是0,1,2...格式
 * @param tdId	checkbox容器id
 */
