...
layui.define(['element', 'common', 'paging', 'form'], function (exports) {
    ...
    BTable.prototype.render = function () {
        ...
        for (var i = 0; i < columns.length; i++) {
        	if(columns[i].hidden){
        		th += '<th style="display:none;">' + columns[i].fieldName + '</th>';
        	}else{
    ...
    function getTpl(options) {
        ...
        for (var i = 0; i < columns.length; i++) {
        	if(columns[i].hidden){
        		tds += '<td style="display:none;">{{ item.' + columns[i].field + ' }}</td>';
        	}else{
        		if(typeof(columns[i].colRender) != 'undefined'){
        			tds += '<td>{{ ' +columns[i].colRender + '(item.' + columns[i].field + ') }}</td>';
        		}else{
        ...
        tpl += '<td align="center" colspan="' + colLength + '">暂无数据.</td>';
        tpl += '{{# } }}';
        return tpl;
    }
